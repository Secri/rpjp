...
function RPJP_dates_disponibles($post_id){
	//l'action ne s'exécute pas si l'on met le post à la corbeille ou si on le restaure
    if(
        isset($_REQUEST['action']) &&
        ( $_REQUEST['action'] == 'trash' || $_REQUEST['action'] == 'untrash')
    ){
	...
	//paramètres pour l'appel de wp_query: on récupère les posts ayant le même post-type et la même catégorie en paramètre et dont les dates se superposeraient
	$args = array(
    'post_type'  => 'regie_publicitaire',
	'post_status' => 'publish',
	'posts_per_page' => -1,
